...
export default function LikedSongs() {
  ...
  return (
    <Box>
      <ContentContainer image={data.UserLikedSongs.items[0].track.album.images[1].url}>

        <GeneralPageLayout data={{name: 'Liked Songs'}} type={'playlist'}>
        </GeneralPageLayout>

        <Box className={classes.playlistContainer}>
          <Tracklist tracks={data.UserLikedSongs.items.map(item => item.track)} isAlbum={false} />
        </Box>
      </ContentContainer>
    </Box>
  )
