...
export default function Artist() {
    ...
    //Get artist data from id in url
    ...
    return (
        <Box>
            <ContentContainer
                image={
                    data.Artist.images.length >= 2
                        ? data.Artist.images[1].url
                        : data.Artist.images[data.Artist.images.length - 1].url
                }
            >
                <GeneralPageLayout
                    data={GetPageLayoutData(data.Artist)}
                    type={'artist'}
                >
                    <b>
                        {data.Artist.followers.total.toLocaleString('en-US')}{' '}
                    </b>{' '}
                    &nbsp;Followers
                </GeneralPageLayout>
                ...
                <Box className={classes.playlistContainer}>
                    <Box className={classes.playButtonContainer}>
                        <Playbutton
                            tracks={data.ArtistTopSongs.tracks}
                            uri={data.Artist.uri}
                        />
                    </Box>
                    <Box className={classes.topic}>Top Tracks</Box>
                    <Tracklist
                        tracks={data.ArtistTopSongs.tracks.slice(0, 5)}
                        isAlbum={false}
                    />
